...
import os
import numpy as np
os.environ['LD_LIBRARY_PATH'] = "/opt/intel/oneapi/2025.3/lib:$LD_LIBRARY_PATH"
...
def run_atlas(settings):
    run_dir = os.path.expanduser(f'/raid/atlas/BasicATLAS_bprp/DFSYNTHE_ODF_{settings.name}')
    shutil.rmtree(run_dir, ignore_errors=True)
    atlas.dfsynthe(run_dir, settings)

    model_dir = os.path.expanduser(f'/raid/atlas/BasicATLAS_bprp/ATLAS_LMHA_{settings.name}')
    ...
    model_dir_upteff = os.path.expanduser(f'/raid/atlas/BasicATLAS_bprp/ATLAS_LMHA_{settings.name}_upteff')
    ...
    model_dir_uplogg = os.path.expanduser(f'/raid/atlas/BasicATLAS_bprp/ATLAS_LMHA_{settings.name}_uplogg')
    ...
    run_dir_upfeh = os.path.expanduser(f'/raid/atlas/BasicATLAS_bprp/DFSYNTHE_ODF_{settings.name}_upfeh')
    shutil.rmtree(run_dir_upfeh, ignore_errors=True)
    atlas.dfsynthe(run_dir_upfeh, settings_upfeh)
    model_dir_upfeh = os.path.expanduser(f'/raid/atlas/BasicATLAS_bprp/ATLAS_LMHA_{settings.name}_upfeh')
    ...
    model_dir_upvt = os.path.expanduser(f'/raid/atlas/BasicATLAS_bprp/ATLAS_LMHA_{settings.name}_upvt')
    shutil.rmtree(model_dir_upvt, ignore_errors=True)
    atlas.atlas(model_dir_upvt, settings_upvt, ODF = run_dir)



def process_single_star(args, He_enhanced=False):
    ...
    settings.name = star
    if He_enhanced: settings.name += '_He_enhanced'
...
def main():
    parser = argparse.ArgumentParser(
        description='Process M15 and M92 stars with ATLAS models',
        formatter_class=argparse.ArgumentDefaultsHelpFormatter
    )
    parser.add_argument('--input', type=str, 
                       default='/raid/caltech/hires/M15_M92_bprp/avg_abund_normal.pkl',
                       help='Path to input pickle file with catalog and abundances')
    parser.add_argument('--output', type=str,
                       default='/raid/caltech/hires/M15_M92_bprp/M15_M92_params_normal.pkl',
                       help='Path to output pickle file for results')
    parser.add_argument('--he-enhanced', action='store_true',
                       help='Use helium enhanced abundances')
    parser.add_argument('--parallel', type=int, default=mp.cpu_count(),
                       help='Number of parallel processes')
    parser.add_argument('--stars', nargs='+', default=None,
                       help='Specific stars to process (optional)')
    ...
    # For stars where N could not be measured, set its abundance to the minimum measured N abundance for that GC.
    for gc_idx in [0, 1]:
        n_idx = np.where(avg_abunds[gc_idx]['element'] == 'N')[0]
        if n_idx.size == 0:
            continue
        nanmask = np.isnan(avg_abunds[gc_idx]['abunddiff'][n_idx].values[0])
        avg_abunds[gc_idx]['abunddiff'][n_idx].values[0][nanmask] = np.min(avg_abunds[gc_idx]['abunddiff'][n_idx].values[0][~nanmask])
    ...
    for j, gc in enumerate(gcs):
        for i, star in enumerate(star_names[j]):
            # Filter by specific stars if requested
            if args.stars is None or star in args.stars:
                #results.append(process_single_star((star, j, i, cat, avg_abunds, names)))
                tasks.append((star, j, i, cat, avg_abunds, names))
    
    print(f"Processing {len(tasks)} stars with {args.parallel} parallel processes")
    
    # Process in parallel
    with Pool(processes=args.parallel) as pool:
        if args.he_enhanced:
            from functools import partial
            process_func = partial(process_single_star, He_enhanced=True)
