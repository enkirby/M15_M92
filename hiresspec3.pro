...
pro hiresspec3::getscience
  ...
  w = where(data.lambda gt 5715 and data.lambda lt 5795 and abs(data.lambda - 5754.656) gt 0.1 and abs(data.lambda - 5762.992) gt 0.1, c)
  if c gt 10 then begin
    ...
    printf, 15, strtrim(name, 2) + ', ' + strtrim(SNR_per_pix, 2) + ', ' + strtrim(SNR_per_ang, 2)
...
function hiresspec3::INIT, catalogfile, indir = indir, outdir = outdir, big = big
  compile_opt idl2
  openw, 15, getenv('CALTECH') + 'hires/M15_M92_bprp/m15_m92_snr.csv'
  printf, 15, 'name,SNR_per_pix,SNR_per_Ang'
...
pro hiresspec3, big = big
  compile_opt idl2
  dir = getenv('CALTECH') + 'hires/M15_M92_bprp/'
